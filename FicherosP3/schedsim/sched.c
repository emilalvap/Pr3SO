...
static inline void lock_rq(runqueue_t* rq) { pthread_mutex_lock(&rq->lock); }
static inline void unlock_rq(runqueue_t* rq){ pthread_mutex_unlock(&rq->lock); }
...
slist_t read_task_list_from_file(FILE *file){
    ...
    init_slist (&slist, sizeof(task_t), offsetof(task_t,ts_links));
...
static const char* strstate[]={"start","wait","run","io","completed"};
...
static void init_task_sched(task_t* task) {
    
    init_slist (&task->sched_regs, sizeof(sched_log_t), offsetof(sched_log_t,links));
    
    if (!(task->flags & TF_IDLE_TASK))
	active_sched_class->task_new(task); //class-specific initialization
}

/* Initialize a per-CPU run queue */
static void init_runqueue(runqueue_t* rq, int cpu) {
    init_slist(&rq->tasks,sizeof(task_t),offsetof(task_t,rq_links));
...
static void sched_init(struct sched_class* sc){
	...
	init_slist(&sched_events[cpu],sizeof(sched_event_t),offsetof(sched_event_t,links));
    }
    
    active_sched_class=sc;
    
    /* Perform class-specific global initialization if required */
    if (active_sched_class->sched_init)
	active_sched_class->sched_init();
